...
portfolioSizes.forEach((sizesElement) => {
	densities.forEach((densitiesElement) =>
		portfolioOutputSizes.push(densitiesElement * sizesElement)
	);
});
...
async function getPostImages(location) {
	...
	for (const directory of directories) {
		...
		const { image, imageAlt: alt } = fmContent.attributes;
	...
	for (let index in postImages) {
		...
		const generatePath = (
			...
			const sources = `[\n${formats
				...
				.join('\n')}\n  ]`;
			return `import meta from '${source}?w=${Math.min(maxWidth, width)}&as=meta:height;src;width';
...
`;
		...
		for (const contentImage of postImage.contentImages ?? []) {
			...
			const { dominantColour, format, placeholder, width } =
				contentImageMetaData[contentImageIndex];
